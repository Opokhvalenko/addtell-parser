...
import type { BidQuery } from "../domain/types.js";
...
// Тип елементу-кандидата (виводимо з репозиторію)
...
export async function pickLineItem(app: FastifyInstance, q: BidQuery) {
  ...
  // первинний відбір по типу та розміру на рівні БД
  const all = await liRepo.findCandidates({
    size: q.size,
    adType: q.type ?? "banner",
  });

  // пайплайн дрібними фільтрами
  ...
  // перевірка частоти показів і повернення першого, що підходить
